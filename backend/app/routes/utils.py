...
# Marcus' function for recommending unis
state_mapping = {
    'NSW': 'New South Wales',
    'QLD': 'Queensland',
    'SA': 'South Australia',
...
# Function to recommend university based on lowest fee and include total cost per year (including cost of living)
def recommend_uni(state, degree, industry, db):
    # Write your query
    ...
    # Convert abbreviated state to full name for cost of living lookup
    full_state = state_mapping.get(state, state)

    # Filter based on the 3 categories (State, Degree Type, and Industry)
    filtered_df = df[(df['State'] == state) & (df['Type'] == degree) & (df['Sector'] == industry)].copy()  # Use .copy() to avoid the warning

    # Calculate yearly fee using .loc
    filtered_df.loc[:, 'Yearly Fee'] = filtered_df['Fee Total'] / filtered_df['Duration']

    # Sort by Yearly Fee and select top 5
    sorted_df = filtered_df.sort_values(by='Yearly Fee').head(5)

    # Find the corresponding cost of living for the state
    cost_of_living = cf[cf['State'] == full_state][['Min Cost', 'Max Cost']].values[0]

    # Prepare the result and add cost of living to the yearly fee
...
# Function to recommend university based on ranking and include total cost per year (including cost of living)
def recommend_uni_rank(state, degree, industry):
    # Convert abbreviated state to full name for cost of living lookup
    full_state = state_mapping.get(state, state)
    ...
    # Calculate yearly fee using .loc to avoid SettingWithCopyWarning
    filtered_df.loc[:, 'Yearly Fee'] = filtered_df['Fee Total'] / filtered_df['Duration']

    # Sort by University Rank and select top 5
    sorted_df = filtered_df.sort_values(by='Uni Rank').head(5)

    # Find the corresponding cost of living for the state
    cost_of_living = cf[cf['State'] == full_state][['Min Cost', 'Max Cost']].values[0]

    # Prepare the result and add cost of living to the yearly fee
    result = sorted_df[['University', 'Degree', 'Yearly Fee', 'Duration', 'Uni Rank']].copy()
    result['Total Min Cost (Yearly Fee + Min Cost)'] = result['Yearly Fee'] + cost_of_living[0]
    result['Total Max Cost (Yearly Fee + Max Cost)'] = result['Yearly Fee'] + cost_of_living[1]
